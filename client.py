from aiogram import Router, F, types, Bot
from aiogram.fsm.context import FSMContext
from aiogram.fsm.state import State, StatesGroup
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
import client_kb as keyboards
import database


client_router = Router()


# ================= HOLATLAR VA QOIDALAR =================
class FastOrder(StatesGroup):
    product_id = State()
    contact = State()
    confirmation = State()


class CartOrder(StatesGroup):
    contact = State()
    confirmation = State()


def get_order_confirmation_kb(order_type):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Buyurtmani tasdiqlash", callback_data=f"confirm_{order_type}")],
        [InlineKeyboardButton(text="🔙 Orqaga", callback_data=f"cancel_{order_type}")]
    ])


def order_policy_text():
    return (
        "🚚 <b>Yetkazib berish:</b> buyurtmani Yandex orqali yuboramiz.\n"
        "Buyurtma yetkazib berilganda qadoq holatini ko'rsatamiz va tekshirish imkonini beramiz.\n\n"
        "💳 <b>To'lov:</b> karta orqali o'tkazma.\n"
        "To'lov bo'yicha admin siz bilan bog'lanadi.\n\n"
        "Quyidagi tugma orqali buyurtmani tasdiqlang."
    )


# ================= BEKOR QILISH =================
@client_router.message(F.text == "🔙 Orqaga", FastOrder.contact)
@client_router.message(F.text == "🔙 Orqaga", CartOrder.contact)
async def cancel_order(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("Buyurtma bekor qilindi.", reply_markup=keyboards.get_user_kb(message.from_user.id))


# ================= KATALOG VA MAHSULOTLAR =================
@client_router.message(F.text == "🛍 Katalog")
async def show_catalog(message: types.Message):
    categories = database.get_all_categories()
    if not categories:
        await message.answer("Hozircha do'konda mahsulotlar yo'q.")
        return
    await message.answer("Bo'limni tanlang:", reply_markup=keyboards.get_categories_kb(categories))


@client_router.callback_query(F.data == "back_to_cats")
async def go_back_to_cats(call: types.CallbackQuery):
    categories = database.get_all_categories()
    await call.message.delete()
    await call.message.answer("Bo'limni tanlang:", reply_markup=keyboards.get_categories_kb(categories))
    await call.answer()


@client_router.callback_query(F.data.startswith("cat_"))
async def show_products_list(call: types.CallbackQuery):
    cat_id = int(call.data.split("_")[1])
    conn = database.db_connect()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name FROM products WHERE category_id = ?", (cat_id,))
    products = cursor.fetchall()
    conn.close()
    if not products:
        await call.message.edit_text("Bu bo'limda hozircha mahsulot yo'q.")
        await call.answer()
        return
    await call.message.edit_text("Kerakli mahsulotni tanlang:", reply_markup=keyboards.get_products_list_kb(products))
    await call.answer()


@client_router.callback_query(F.data.startswith("prod_"))
async def show_one_product(call: types.CallbackQuery):
    prod_id = int(call.data.split("_")[1])
    conn = database.db_connect()
    cursor = conn.cursor()
    cursor.execute("SELECT name, description, price, photo_id, is_available, category_id FROM products WHERE id = ?",
                   (prod_id,))
    prod = cursor.fetchone()
    conn.close()

    if not prod:
        await call.answer("❌ Mahsulot topilmadi!", show_alert=True)
        return

    status_text = "🟢 Sotuvda bor" if prod[4] else "🔴 Qolmagan (Vaqtinchalik yo'q)"
    caption = f"🏷 <b>Mahsulot:</b> {prod[0]}\n\n📋 <b>Tavsif:</b> {prod[1]}\n\n💵 <b>Narxi:</b> {prod[2]} so'm\n\n📌 <b>Holati:</b> {status_text}"

    await call.message.delete()
    await call.message.answer_photo(photo=prod[3], caption=caption, parse_mode="HTML",
                                    reply_markup=keyboards.get_product_actions_kb(prod_id, prod[4], prod[5]))
    await call.answer()


@client_router.callback_query(F.data.startswith("backtoprods_"))
async def back_to_prods_list(call: types.CallbackQuery):
    cat_id = int(call.data.split("_")[1])
    conn = database.db_connect()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name FROM products WHERE category_id = ?", (cat_id,))
    products = cursor.fetchall()
    conn.close()
    await call.message.delete()
    if not products:
        await call.message.answer("Bu bo'limda hozircha mahsulot yo'q.")
    else:
        await call.message.answer("Kerakli mahsulotni tanlang:", reply_markup=keyboards.get_products_list_kb(products))
    await call.answer()


@client_router.callback_query(F.data == "ignore")
async def ignore_action(call: types.CallbackQuery):
    await call.answer("❌ Bu mahsulot vaqtinchalik sotuvda yo'q, xarid qila olmaysiz!", show_alert=True)


# ================= SAVATCHA =================
@client_router.callback_query(F.data.startswith("add_"))
async def add_cart(call: types.CallbackQuery):
    prod_id = int(call.data.split("_")[1])
    database.add_to_cart(call.from_user.id, prod_id)
    await call.answer("✅ Mahsulot savatchaga qo'shildi!", show_alert=True)


@client_router.message(F.text == "🛒 Savatcha")
async def view_cart(message: types.Message):
    items = database.get_cart(message.from_user.id)
    if not items:
        await message.answer("Savatchangiz hozircha bo'm-bo'sh 😔")
        return
    text = "🛒 <b>Sizning savatchangiz:</b>\n\n"
    total_price = 0
    for i, item in enumerate(items, 1):
        text += f"{i}. {item[0]} — {item[1]} so'm\n"
        total_price += item[1]
    text += f"\n💵 <b>Jami to'lov: {total_price} so'm</b>"
    await message.answer(text, parse_mode="HTML", reply_markup=keyboards.get_cart_kb())


@client_router.callback_query(F.data == "clear_cart")
async def clear_cart_action(call: types.CallbackQuery):
    database.clear_cart(call.from_user.id)
    await call.message.edit_text("🗑 Savatcha tozalandi.")
    await call.answer()


# ================= SAVATCHADAN BUYURTMA =================
@client_router.callback_query(F.data == "checkout_cart")
async def checkout_cart_start(call: types.CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer("📞 Buyurtmani rasmiylashtirish uchun telefon raqamingizni yuboring:",
                              reply_markup=keyboards.contact_kb)
    await state.set_state(CartOrder.contact)
    await call.answer()


@client_router.message(CartOrder.contact)
async def cart_buy_finish(message: types.Message, state: FSMContext):
    contact = message.contact.phone_number if message.contact else message.text
    items = database.get_cart(message.from_user.id)
    if not items:
        await state.clear()
        await message.answer("Savatchangiz bo'sh.", reply_markup=keyboards.get_user_kb(message.from_user.id))
        return
    total_price = 0
    order_list = ""
    for i, item in enumerate(items, 1):
        order_list += f"{i}. {item[0]} — {item[1]} so'm\n"
        total_price += item[1]
    await state.update_data(contact=contact)
    await state.set_state(CartOrder.confirmation)
    confirmation_text = f"🧾 <b>BUYURTMANI TEKSHIRING</b>\n\n📦 <b>Mahsulotlar:</b>\n{order_list}💵 <b>Jami:</b> {total_price} so'm\n📞 <b>Telefon:</b> {contact}\n\n{order_policy_text()}"
    await message.answer(confirmation_text, parse_mode="HTML", reply_markup=types.ReplyKeyboardRemove())
    await message.answer("Buyurtmani yuborishga tayyormisiz?", reply_markup=get_order_confirmation_kb("cart"))


@client_router.callback_query(F.data == "confirm_cart", CartOrder.confirmation)
async def confirm_cart_order(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    data = await state.get_data()
    contact = data.get("contact")
    items = database.get_cart(call.from_user.id)
    if not contact or not items:
        await state.clear()
        await call.message.edit_text("❌ Buyurtma topilmadi. Qaytadan urinib ko'ring.")
        await call.answer()
        return
    total_price = 0
    order_list = ""
    for i, item in enumerate(items, 1):
        order_list += f"{i}. {item[0]} — {item[1]} so'm\n"
        total_price += item[1]
    username = f"@{call.from_user.username}" if call.from_user.username else "Yashiringan"
    admin_text = f"🚨 <b>YANGI ZAKAZ (Savatchadan)!</b>\n\n👤 <b>Xaridor:</b> {call.from_user.first_name}\n📞 <b>Raqami:</b> {contact}\n🔗 <b>Profili:</b> {username}\n\n📦 <b>Mahsulotlar:</b>\n{order_list}💵 <b>Jami to'lov:</b> {total_price} so'm"

    admin_ids = database.get_all_admins()
    sent_count = 0
    for admin_id in admin_ids:
        try:
            await bot.send_message(admin_id, admin_text, parse_mode="HTML")
            sent_count += 1
        except:
            pass

    database.clear_cart(call.from_user.id)
    await state.clear()

    if sent_count == 0:
        await call.message.edit_text("⚠️ Buyurtma tasdiqlandi, lekin adminlarga xabar yuborilmadi.")
    else:
        await call.message.edit_text(
            "✅ <b>Buyurtmangiz tasdiqlandi!</b>\n\nTez orada adminlarimiz siz bilan bog'lanadi.", parse_mode="HTML")
    await call.answer("Tasdiqlandi!")


@client_router.callback_query(F.data == "cancel_cart", CartOrder.confirmation)
async def cancel_cart_order(call: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await call.message.edit_text("↩️ Buyurtma bekor qilindi.")
    await call.message.answer("Asosiy menyu:", reply_markup=keyboards.get_user_kb(call.from_user.id))
    await call.answer("Bekor qilindi")


# ================= TEZKOR BUYURTMA =================
@client_router.callback_query(F.data.startswith("buy_"))
async def fast_buy_start(call: types.CallbackQuery, state: FSMContext):
    prod_id = int(call.data.split("_")[1])
    await state.update_data(product_id=prod_id)
    await call.message.delete()
    await call.message.answer("📞 Buyurtmani rasmiylashtirish uchun telefon raqamingizni yuboring:",
                              reply_markup=keyboards.contact_kb)
    await state.set_state(FastOrder.contact)
    await call.answer()


@client_router.message(FastOrder.contact)
async def fast_buy_finish(message: types.Message, state: FSMContext):
    contact = message.contact.phone_number if message.contact else message.text
    data = await state.get_data()
    prod_id = data.get("product_id")
    if not prod_id:
        await state.clear()
        await message.answer("❌ Xatolik. Qaytadan urinib ko'ring.",
                             reply_markup=keyboards.get_user_kb(message.from_user.id))
        return

    conn = database.db_connect()
    cursor = conn.cursor()
    cursor.execute("SELECT name, price FROM products WHERE id = ?", (prod_id,))
    prod = cursor.fetchone()
    conn.close()

    if not prod:
        await state.clear()
        await message.answer("❌ Mahsulot topilmadi!", reply_markup=keyboards.get_user_kb(message.from_user.id))
        return

    await state.update_data(contact=contact)
    await state.set_state(FastOrder.confirmation)

    confirmation_text = f"🧾 <b>BUYURTMANI TEKSHIRING</b>\n\n📦 <b>Mahsulot:</b> {prod[0]}\n💵 <b>Narxi:</b> {prod[1]} so'm\n📞 <b>Telefon:</b> {contact}\n\n{order_policy_text()}"
    await message.answer(confirmation_text, parse_mode="HTML", reply_markup=types.ReplyKeyboardRemove())
    await message.answer("Buyurtmani yuborishga tayyormisiz?", reply_markup=get_order_confirmation_kb("fast"))


@client_router.callback_query(F.data == "confirm_fast", FastOrder.confirmation)
async def confirm_fast_order(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    data = await state.get_data()
    contact = data.get("contact")
    prod_id = data.get("product_id")

    if not contact or not prod_id:
        await state.clear()
        await call.message.edit_text("❌ Xatolik yuz berdi.")
        return

    conn = database.db_connect()
    cursor = conn.cursor()
    cursor.execute("SELECT name, price FROM products WHERE id = ?", (prod_id,))
    prod = cursor.fetchone()
    conn.close()

    if not prod:
        await state.clear()
        await call.message.edit_text("❌ Mahsulot topilmadi.")
        return

    username = f"@{call.from_user.username}" if call.from_user.username else "Yashiringan"
    admin_text = f"🚨 <b>YANGI ZAKAZ (Tezkor)!</b>\n\n👤 <b>Xaridor:</b> {call.from_user.first_name}\n📞 <b>Raqami:</b> {contact}\n🔗 <b>Profili:</b> {username}\n\n📦 <b>Mahsulot:</b> {prod[0]}\n💵 <b>Narxi:</b> {prod[1]} so'm"

    admin_ids = database.get_all_admins()
    sent_count = 0
    for admin_id in admin_ids:
        try:
            await bot.send_message(admin_id, admin_text, parse_mode="HTML")
            sent_count += 1
        except:
            pass

    await state.clear()

    if sent_count == 0:
        await call.message.edit_text("⚠️ Buyurtma tasdiqlandi, lekin adminlarga xabar yuborilmadi.")
    else:
        await call.message.edit_text(
            "✅ <b>Buyurtmangiz tasdiqlandi!</b>\n\nTez orada adminlarimiz siz bilan bog'lanadi.", parse_mode="HTML")
    await call.answer("Tasdiqlandi!")


@client_router.callback_query(F.data == "cancel_fast", FastOrder.confirmation)
async def cancel_fast_order(call: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await call.message.edit_text("↩️ Buyurtma bekor qilindi.")
    await call.message.answer("Asosiy menyu:", reply_markup=keyboards.get_user_kb(call.from_user.id))
    await call.answer("Bekor qilindi")