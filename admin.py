...
# Yangi narsa qo'shish statelari
class AddCategory(StatesGroup): name = State()


class AddProduct(
    StatesGroup): category_id = State(); name = State(); description = State(); price = State(); photo = State()


# Tahrirlash statelari
class EditCatName(StatesGroup): cat_id = State(); new_name = State()


class EditProdName(StatesGroup): prod_id = State(); new_name = State()


class EditProdDesc(StatesGroup): prod_id = State(); new_desc = State()


class EditProdPhoto(StatesGroup): prod_id = State(); new_photo = State()


@admin_router.message(F.text == "🔙 Orqaga")
async def cancel_action(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("Jarayon bekor qilindi. Boshqaruv paneli:", reply_markup=keyboards.admin_kb)


@admin_router.message(F.text == "⚙️ Admin panel")
async def admin_panel(message: types.Message):
    if database.is_admin(message.from_user.id):
        await message.answer("Boshqaruv paneli", reply_markup=keyboards.admin_kb)
...
async def add_cat_name(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"✅ '{message.text}' qo'shildi!", reply_markup=keyboards.admin_kb)
...
async def add_product_start(message: types.Message, state: FSMContext):
    if database.is_admin(message.from_user.id):
        ...
        if not categories:
            await message.answer("❌ Hali bo'lim yo'q!", reply_markup=keyboards.admin_kb)
            return
        await message.answer("Qaysi bo'limga qo'shamiz?",
                             reply_markup=keyboards.get_manage_cats_list_kb(categories, "admincat_"))
...
async def add_product_photo(message: types.Message, state: FSMContext):
    ...
    conn.close()
    await state.clear()
    await message.answer(f"✅ Mahsulot qo'shildi!", reply_markup=keyboards.admin_kb)
...
async def edit_cat_name_finish(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Kategoriya nomi o'zgartirildi!", reply_markup=keyboards.admin_kb)
...
async def edit_prod_name_finish(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Mahsulot nomi o'zgartirildi!", reply_markup=keyboards.admin_kb)
...
async def edit_prod_desc_finish(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Mahsulot tavsifi o'zgartirildi!", reply_markup=keyboards.admin_kb)
